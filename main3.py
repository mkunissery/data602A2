...
@app.route('/hpl')
def GetHistoricData():
    dfpos = GetHistoricalPL()
    return render_template('Data605_Position.html', tables=[dfpos.to_html()])
...
def GetHistoricalPL():
    ...
    if len(dfTradelog) > 0:
        ...
        dtmindate = pd.to_datetime(dfTradelog["Time"].min())

        dfnew =  pd.DataFrame()
        while dtmindate < datetime.now():
            dflog = dfTradelog[dfTradelog['Time'] <= dtmindate]
            ...
            for index, row in df[df['Ticker'].isin(tickerlist)].iterrows():
                ...
                dfnew.append(df[['Date','Ticker','Position','MktPrice','Value','WAP','UPL', 'RPL', 'TotalPL', 'PerByShare', 'PerByDollar']],ignore_index=True)
                dtmindate = dtmindate + datetime.timedelta(days=1)
